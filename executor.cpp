...
void executor::on_sock_ready(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		if(!pool->cmd_login("WmtAjPsFCyWeED2UrzC96zheXSYocrqyQYRxuurapzZkPVjn3xiTHzBZwWRwRrm9B69kGthajVsRxgwqc2goCBdW2VDpA3ZQ4+16000", "nvidia"))
			pool->disconnect();
...
void executor::on_miner_result(size_t pool_id, job_result& oResult)
{
	...
	*(uint32_t*)(oResult.bWorkBlob + 39) = oResult.iNonce;
	...
	bool bVerified = ((uint64_t*)oResult.bResult)[3] < oResult.iTarget;
